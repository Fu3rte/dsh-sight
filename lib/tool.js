...
export function buildVisionTool(config, toolName) {
  const backendLine = config.ready
    ...
    : `NO backend configured yet: provider "${config.provider}" is missing its API key or base URL. Set ${config.preset.keyEnv || 'DSH_SIGHT_API_KEY'} or fill ~/.config/dsh-sight/config.json.`
  ...
  return {
    ...
    async execute(args, exec) {
      ...
      if (!config.ready) {
        throw new Error(
          `No vision backend configured. provider="${config.provider}", apiKey ${config.apiKey ? 'set' : 'MISSING'}, baseUrl ${config.baseUrl ? 'set' : 'MISSING'}, model ${config.model ? 'set' : 'MISSING'}. Set ${config.preset.keyEnv || 'DSH_SIGHT_API_KEY'} or write ~/.config/dsh-sight/config.json.`,
        )
      }
