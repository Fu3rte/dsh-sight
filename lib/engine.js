// Vision engine: OpenAI-compatible backend, one batched entry point.
//
// One POST {baseUrl}/chat/completions carries every image as an inline
// base64 image_url part — a multi-image batch is a single request, and the
// result labels each description.
//
// Inputs are local absolute paths or http(s) URLs. Both local and remote
// bodies are capped at 25 MiB; URL fetches additionally get a 30s timeout and
// must claim an image/* content type. Remote bodies are inlined, never
// forwarded as URLs (no SSRF surface on the vision API). Keyless presets send
...
/**
 * Resolve every input into { dataUrl }. Rejections are collected per-image;
 * a batch fails only when nothing resolves.
 */
...
/**
 * Analyze one or more images in a single request. Returns a plain-text
 * description; per-image load failures are appended as bracketed notes so
 * partial batches still report what they could see.
 */
