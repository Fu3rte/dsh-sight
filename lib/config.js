...
//   1. environment variables (DSH_SIGHT_*)
//   2. config file  (~/.config/dsh-sight/config.json, 0600; path
//      overridable via DSH_SIGHT_CONFIG)
//   3. plugin row config (cordis.patch.yml `config:` block)
//   4. preset defaults
//
// The file is re-read when its mtime changes, so `dsh-sight doctor`-
...
export function resolveConfig(rowConfig = {}) {
  ...
  let provider = pick(envValue('DSH_SIGHT_PROVIDER'), raw('provider'), 'string')
  ...
  for (const field of NUM_FIELDS) {
    config[field] = pick(config[field], envValue(`DSH_SIGHT_${camelToUpper(field)}`), 'number') ?? pick(config[field], raw(field), 'number')
  }
  for (const field of BOOL_FIELDS) {
    const env = envValue(`DSH_SIGHT_${camelToUpper(field)}`)
  ...
  for (const field of STR_FIELDS) {
    const value = pick(envValue(`DSH_SIGHT_${camelToUpper(field)}`), raw(field), 'string')
  ...
  if (config.apiType && config.apiType !== 'openai' && config.apiType !== 'minimax') {
    console.error(`[dsh-sight] unknown apiType "${config.apiType}"; falling back to the preset's type`)
  ...
  const genericEnv = envValue('DSH_SIGHT_API_KEY')
