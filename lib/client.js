...
//
// Form controls are the host design system's own atoms (Menu / Input / Button
// from @deepseek-ai/dsh-client-ui-primitives), so dropdown lists and inputs
// follow the active theme — native <select> popups ignore the dark theme.
//
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const PRESET_OPTIONS = Object.keys(CLIENT_PRESETS).map((id) => ({
      id,
      label: (id !== 'custom' && CLIENT_PRESETS[id].keyless ? '免 key · ' : '') + CLIENT_PRESETS[id].label,
    }))
    const APITYPE_OPTIONS = [
      { id: 'openai', label: 'openai（OpenAI 兼容 /chat/completions）' },
      { id: 'minimax', label: 'minimax（MiniMax 原生 VLM 端点）' },
    ]
    ...
    function DshSightSection(props) {
      const api = props.api
      const [form, setForm] = useState({ provider: 'opencode-zen', model: '', baseUrl: '', apiType: 'openai', timeoutMs: '120000', maxTokens: '4096' })
      ...
      return react.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '8px', padding: '0 4px' } }, [
        ...
        react.createElement('details', { key: 'adv' }, [
          ...
          react.createElement(Field, { key: 'apiType', label: 'API 类型' }, [
            react.createElement(MenuField, { key: 'm', value: form.apiType, options: APITYPE_OPTIONS, onChange: (apiType) => setForm((p) => ({ ...p, apiType })) }),
          ]),
        ...
        react.createElement('p', { key: 'note', style: hintStyle },
          '免 key 方案：OpenCode Zen 免费层（免注册但高峰期可能限流）或本地 Ollama（ollama run qwen2.5vl:7b 后直接可用）。Gemini Flash 为免费额度，只需 Google AI Studio 的注册 key。粘贴图片时 dsh-sight 会先把图片存到本地并注入路径提示，文本模型随后调用 vision 工具读取。多图可一次传：vision(paths=[a.png, b.png])。'),
      ])
    }
