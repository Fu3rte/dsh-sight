    ...
    function DshSightSection(props) {
      ...
      return react.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '8px', padding: '0 4px' } }, [
        ...
        react.createElement('p', { key: 'note', style: hintStyle },
          '免 key 方案：OpenCode Zen 免费层（免注册但高峰期可能限流）。Gemini Flash 为免费额度，只需 Google AI Studio 的注册 key。粘贴图片时 dsh-sight 会先把图片存到本地并注入路径提示，文本模型随后调用 vision 工具读取。多图可一次传：vision(paths=[a.png, b.png])。'),
      ])
    }
