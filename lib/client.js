...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CLIENT_PRESETS = {
      'glm-4v-flash': { label: 'GLM-4V-Flash — Zhipu BigModel (free tier)', model: 'glm-4v-flash', baseUrl: 'https://open.bigmodel.cn/api/paas/v4', apiType: 'openai', keyless: false, keyEnv: 'ZHIPU_API_KEY' },
      'mimo-v2.5': { label: 'MiMo-V2.5 — MiniMax (¥1 / ¥2 per M tokens)', model: 'MiMo-V2.5', baseUrl: 'https://api.minimax.io', apiType: 'minimax', keyless: false, keyEnv: 'MINIMAX_API_KEY' },
      ...
      'gemini-flash': { label: 'Gemini Flash — Google AI Studio (free tier)', model: 'gemini-3.6-flash', baseUrl: 'https://generativelanguage.googleapis.com/v1beta/openai', apiType: 'openai', keyless: false, keyEnv: 'GEMINI_API_KEY' },
      'opencode-zen': { label: 'OpenCode Zen (free tier, keyless)', model: 'mimo-v2.5-free', baseUrl: 'https://opencode.ai/zen/v1', apiType: 'openai', keyless: true, keyEnv: null },
      'opencode-zen-go': { label: 'OpenCode Zen Go ($10/mo: mimo-v2.5, minimax-m3, …)', model: 'mimo-v2.5', baseUrl: 'https://opencode.ai/zen/go/v1', apiType: 'openai', keyless: false, keyEnv: 'ZEN_API_KEY' },
    ...
    function Field(props) {
      return react.createElement('div', { style: { marginBottom: '12px' } }, [
        ...
        react.createElement(props.tag || 'input', {
          ...
          ? props.options.map((option) => react.createElement('option', { key: option.id, value: option.id }, option.label))
          : undefined),
      ])
    }
    ...
    function DshSightSection(props) {
      ...
      return react.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '8px', padding: '0 4px' } }, [
        react.createElement('p', { key: 'status', style: statusStyle },
          backendReady ? `✓ 后端就绪：${preset.label}（${form.model || '—'}）` : '未就绪：选择预设并填写 API key 后保存'),
        ...
        react.createElement('p', { key: 'note', style: hintStyle },
          '粘贴图片时 dsh-sight 会先把图片存到本地并注入路径提示，文本模型随后调用 vision 工具读取。多图可一次传：vision(paths=[a.png, b.png])。'),
      ])
    }
